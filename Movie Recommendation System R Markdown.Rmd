...
```{r partitioning test and training sets 3}
removed <- anti_join(temp, test_set)
train_set <- rbind(train_set, removed)
```

```{r removing now useless objects from environment, echo=FALSE}
...
```{r user effects model 3, message = FALSE}
...
```{r regularization only of movie effects}
